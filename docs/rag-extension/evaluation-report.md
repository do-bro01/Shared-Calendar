...
> **iter 3 — 측정 보정판 + Baseline 비교 + AnswerSimilarity 추가** (2026-06-05) · 평가자: gpt-4o
...
## 1. 결과 요약 (iter 3, 5 메트릭)

15개 질문 RAG (chat-rag) 평균. iter 별 비교 + AnswerSimilarity 신규.

| 메트릭 | iter 1 | iter 2 | **iter 3** | 목표 | 달성 |
|---|---:|---:|---:|---:|:---:|
| Faithfulness (사실 일치도) | 0.749 | 0.807 | **0.864** | ≥ 0.85 | **✓** |
| Answer Relevancy (답변 관련성) | 0.397 | 0.393 | 0.386 | ≥ 0.80 | ✗ (측정 한계) |
| **Answer Similarity** (정답 임베딩) | — | — | **0.701** | ≥ 0.80 | 근접 |
...
> **iter 3 변경**:
> - (a) 평가 LLM `gpt-4o-mini` → `gpt-4o`
> - (b) `--include-meta` — chat-rag 답변에 등장한 메타 채널 일정을 `retrieved_contexts` 에 추가
> - (c) **AnswerSimilarity 추가** — Ans.Rel 의 한국어 측정 노이즈 (§9.5) 를 우회. 답변과 reference 의 임베딩 코사인 유사도. **0.701** 로 Ans.Rel 0.386 과는 다른 정직한 그림.
>
> 챗봇은 iter 2 그대로, 측정만 더 정확. iter 2 응답을 재사용.
>
> **Faithfulness 0.864 로 목표 0.85 돌파** (gpt-4o 평가자 변동성으로 0.86~0.97 사이 운동). Ctx. Recall 0.760 목표 근접. **AnswerSimilarity 0.701 이 챗봇의 진짜 답변 품질을 보여줌** — Ans.Rel 의 0.39 는 측정 한계지 챗봇 한계가 아님.
...
| 카테고리 | Faith. | Ans. Rel. | **Ans. Sim.** | Ctx. Prec. | Ctx. Recall |
|---|---:|---:|---:|---:|---:|
| 사실(날짜/장소) | 0.917 | 0.395 | **0.793** | **0.825** | **1.000** |
| 사실(인물/관계) | 0.667 | 0.344 | 0.604 | 0.370 | 0.333 |
| 통계/패턴 | **0.933** | **0.506** | 0.645 | 0.333 | **0.833** |
| 시간 한정 회상 | 0.889 | 0.290 | 0.669 | 0.333 | 0.635 |

**핵심 관측**:
- **AnswerSimilarity 가 4 카테고리 모두 0.60+** — Ans.Rel 의 카테고리별 편차 (0.29~0.51) 와 달리 일관적. Ans.Rel 의 변동 다수가 측정 노이즈임을 시사.
- 사실(날짜/장소) 가 모든 메트릭에서 최강 — 단일 일정 응답이 RAG 와 평가 메트릭 양쪽에 가장 잘 맞는 형태.
- 사실(인물/관계) 의 Ctx. Precision/Recall 0.37/0.33 은 여전히 약점 — 임베딩이 "셋이" "등산" 같은 추상 조건을 잘 못 잡음.

**메트릭 변동성 주의**: Faith·Ans.Rel·Ctx.Prec·Ctx.Recall 은 모두 LLM (gpt-4o) 평가 → 같은 입력으로 재실행 시 ±0.05~0.10 변동. 이전 실행에선 Faith 가 0.970 까지 나오기도 함. AnswerSimilarity 는 임베딩 기반이라 결정적이고 변동 없음.
...
### 6.2 결과: RAG 가 5 메트릭 모두 우세

![comparison](../../scripts/eval_outputs/comparison_baseline_vs_rag.png)

| 메트릭 | Baseline | RAG | Δ |
|---|---:|---:|---:|
| Faithfulness | 0.716 | **0.864** | **+0.148** |
| Answer Relevancy | 0.341 | 0.386 | +0.045 |
| **Answer Similarity** | 0.611 | **0.701** | **+0.090** |
| Context Precision | 0.500 | 0.537 | +0.037 |
| Context Recall | 0.698 | **0.760** | +0.062 |

**핵심 발견**:
- 가장 큰 차이는 **Faithfulness (+0.148)**. RAG 가 환각·노이즈를 크게 줄임. Baseline 은 200건 dump 중 잘못된 일정을 끌어쓰는 경우가 잦음.
- **Answer Similarity 도 +0.090 의 의미 있는 우위** — 측정 노이즈 없는 임베딩 기반 메트릭에서도 RAG 가 reference 와 더 가까운 답변을 생성. Ans.Rel 노이즈 의심을 정량적으로 해소.
...
> "vector + 메타 RAG 는 순수 LLM 대비 **Faithfulness +0.148**, **Answer Similarity +0.090** 의 정량 우위. 특히 인물/관계·시간 한정 질문에서 환각을 크게 줄임. 단순 시간 범위 질문에서는 두 시스템이 비등하므로 진짜 가치는 시맨틱 매칭 카테고리에 있음."

### 6.7 AnswerSimilarity 가 보여주는 그림

| 시스템 | Ans. Relevancy (LLM 노이즈 큼) | **Ans. Similarity** (임베딩 기반) |
|---|---:|---:|
| Baseline | 0.341 | 0.611 |
| RAG | 0.386 | **0.701** |

Ans. Rel. 의 절대값 0.34~0.39 만 보면 두 시스템 모두 "답변 품질 0.4" 처럼 보임 (§9.5). AnswerSimilarity 로 측정하면 두 시스템 모두 **0.6+ 의 합리적 수준**이고, RAG 가 reference 에 더 가까운 답변을 생성한다는 사실이 명확히 드러남. 발표 자료엔 두 메트릭을 같이 제시하는 것이 정직함.

---

## 7. 질문별 상세 (iter 3, 5 메트릭)

![per-question](../../scripts/eval_outputs/ragas_per_question.png)

| Q | 카테고리 | Faith. | Ans.R. | **Ans.Sim.** | Ctx.P. | Ctx.R. |
|---:|---|---:|---:|---:|---:|---:|
| 1 | 날짜/장소 | 0.500 | 0.358 | **0.848** | 0.200 | 1.000 |
| 2 | 날짜/장소 | 1.000 | **0.711** | 0.796 | 1.000 | 1.000 |
| 3 | 날짜/장소 | 1.000 | 0.119 | **0.907** | 0.750 | 1.000 |
| 4 | 날짜/장소 | 1.000 | 0.580 | 0.806 | 1.000 | 1.000 |
| 5 | 날짜/장소 | 1.000 | 0.169 | 0.656 | 1.000 | 1.000 |
| 6 | 날짜/장소 | 1.000 | 0.433 | 0.743 | 1.000 | 1.000 |
| 7 | 인물/관계 | 0.000 | 0.258 | 0.694 | 0.111 | 0.000 |
| 8 | 인물/관계 | 1.000 | 0.517 | 0.393 | 1.000 | 0.500 |
| 9 | 인물/관계 | 1.000 | 0.256 | 0.726 | 0.000 | 0.500 |
| 10 | 통계/패턴 | 0.800 | 0.478 | 0.727 | 0.000 | 0.500 |
| 11 | 통계/패턴 | 1.000 | **0.776** | 0.539 | 1.000 | 1.000 |
| 12 | 통계/패턴 | 1.000 | 0.266 | 0.667 | 0.000 | 1.000 |
| 13 | 시간 회상 | 1.000 | 0.206 | 0.743 | 0.000 | 0.333 |
| 14 | 시간 회상 | 0.667 | 0.554 | 0.621 | 0.000 | 0.571 |
| 15 | 시간 회상 | 1.000 | 0.111 | 0.644 | 1.000 | 1.000 |

**관찰**:
- AnswerSimilarity 가 Ans.Rel 의 노이즈를 정리: Q3 ("작년 여름 여행") 은 Ans.Rel **0.119** 인데 Ans.Sim **0.907** — 답변이 reference 와 매우 비슷한데 RAGAS 의 후보-질문 역생성이 실패한 케이스. Ans.Sim 이 진짜 답변 품질을 보여줌.
- Q8 ("지수 생일") 은 반대 — Ans.Sim 0.393 으로 낮음. 답변이 "3월 12일" 만 말하고 reference 의 "성수 비건바 디너 파티" 부분을 빼서 표현 거리가 큼. 즉 **Ans.Sim 은 빠뜨림에 민감, Ans.Rel 은 추가 정보에 민감** — 두 메트릭이 상보적.
...
### 9.6 AnswerSimilarity 적용 결과 (iter 3+ 검증)

위에서 제안한 `AnswerSimilarity` 를 iter 3 에 실제 적용한 결과:

| 메트릭 | RAG iter 3 | Baseline | 챗봇 품질 해석 |
|---|---:|---:|---|
| Ans. Relevancy (구) | 0.386 | 0.341 | 두 시스템 모두 "0.4 수준" 처럼 보이는 측정 노이즈 |
| **Ans. Similarity (신)** | **0.701** | 0.611 | 챗봇이 reference 와 임베딩 거리 0.3 이내로 가까운 답 생성 |

AnswerSimilarity 가 **임베딩 기반이라 LLM n=1 이슈 없음**, 결정적 (run-to-run 변동 없음). 두 메트릭이 같은 방향을 가리키므로 (RAG > Baseline) Ans.Rel 의 상대 비교는 신뢰 가능, 단 **절대값은 Ans. Similarity 가 더 정직함**.
...
3. ~~**Baseline 비교군 평가**~~ — iter 3 적용 완료 (§6). RAG +0.148 Faith / +0.090 AnsSim 우위.
4. ~~**AnswerSimilarity 추가**~~ — iter 3+ 적용 완료. 0.701 로 Ans.Rel 의 측정 한계 해소.
5. **Tag 필터링 + Hybrid Search** (1~2일) — `events.tags` 활용 + BM25. Q7·Q9·Q10 의 Ctx.Precision 0.0~0.11 을 끌어올릴 유일한 방법. 인물/관계 Ctx +0.3 기대.
6. **top-k 12~16** (1시간) — Q1·Q10·Q13·Q14 의 답변 누락 보완.
